...
public class ApproveReviewActionJob implements AutomatedWorkflowTemplateJob, de.hybris.platform.workflow.jalo.AutomatedWorkflowTemplateJob {
			...
			for (final ItemModel item : workflowActionModel.getAttachmentItems()) {
				if (item instanceof CustomerReviewModel) {
					...
					LOG.info(review.getUser().getUid() + "'s review (" + review.getHeadline() + ") of product '" + review.getProduct().getName() + "' ("
							+ review.getProduct().getCode() + ") has been approved.");

					final EmailTemplateModel template = getEmailTemplateService().getByCode("review_approved_email_template");
					final Map<String, Object> data = new HashMap<String, Object>();
					data.put("review", review);
					data.put("product", review.getProduct());
					data.put("user", review.getUser());
					data.put("url", "http://localhost:9001/frameit/product/" + review.getProduct().getCode());
